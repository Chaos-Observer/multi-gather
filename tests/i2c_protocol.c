...
void writeReg(uint8_t i2cAddress, uint8_t reg, uint8_t *pBuf, uint16_t len)
{
    ...
    // printf("write,ADDR:%02x,reg:%02x,data:%02x,len:%d\n",i2cAddress,reg,*pBuf*256+*(pBuf+1),len);
    struct i2c_msg msgs[1] =
    {
            /* Write len+1-bit address */
            { .addr = i2cAddress, .flags = 0, .len = len+1, .buf = data },//ptr type?
...
void readReg(uint8_t i2cAddress, uint8_t reg, uint8_t *pBuf, uint16_t len)
{
    ...
    struct i2c_msg msgs[2] =
    {
            ...
            { .addr = i2cAddress, .flags = 0, .len = 1, .buf = &reg_ },
            /* Read 8-bit data */
            { .addr = i2cAddress, .flags = I2C_M_RD, .len = len, .buf = pBuf},
    };

	i2c_transfer(i2c, msgs, 2);
    // printf("read,ADDR:%02x,reg:%02x,data:%02x,len:%d\n",i2cAddress,reg,*pBuf*256+*(pBuf+1),len);
}
